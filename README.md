...
- **缩放窗口**：拖动右下角的斜线缩放手柄
- **最小尺寸**：窗口不能缩小到单个节点卡片无法完整显示的尺寸
- **锁定窗口**：点击右上角“开/锁”按钮，未锁定为蓝色，锁定后变红；锁定后不能拖动或缩放窗口
- **立即刷新**：点击右上角 ↻ 按钮
- **最小化**：点击右上角 — 按钮
- **关闭**：点击右上角 × 按钮
...
从 `v1.0.8` 开始，应用内在线更新会使用静默安装模式。用户在应用内确认更新后，程序会自动下载最新版安装器、后台覆盖安装到当前目录，并在安装完成后重新启动应用，不再弹出安装器让用户二次点击“安装”。锁定窗口后也会同时禁止拖动和缩放。

从 `v1.0.9` 开始，应用和安装器使用自定义圆形图标，桌面快捷方式会显示同款图标；锁定按钮也改为明显的颜色状态，未锁定为蓝色“开”，锁定后为红色“锁”。
